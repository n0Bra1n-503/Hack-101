...
> AI-powered weather sensor reliability and decision-intelligence system.
...
| :--- | :--- | :--- | :--- |
| **Manan** | ML + EDA | **Anomaly Detection** | Statistical anomaly detection, Isolation Forest, Autoencoders, hybrid anomaly models, feature engineering, ML-oriented EDA, and anomaly evaluation. |
...
| **Mitali** | ML + Backend | **Decision Intelligence + Backend Integration** | Integrates anomaly outputs via adapters, develops evidence gathering (temporal, cross-sensor, cross-station), persistence, weather-vs-sensor decision engine, trust scoring, fault classification, explanations, corrections, digital twin, maintenance prioritization, APIs, and cascade simulation. |
| **Medhvi** | Frontend + EDA | **Visual Analytics + Information Architecture** | Dashboard UX structure, visual analytics, time-series charts, spatial station comparisons, Trust Score visualization, Digital Twin health profiles, maintenance queues, and cascade visualization design. |
...
Mitali's module centers on **Decision Intelligence and Backend Integration**.

### Important Boundary:
Mitali does **NOT** own or train Manan's anomaly detection models. Manan's models answer:
> *"Does this reading look unusual?"*

Mitali's pipeline takes over after anomaly detection to answer:
> *"What does this anomaly actually mean, can it be trusted, why or why not, and what operational action should be taken?"*

Mitali's subsystem encapsulates:
* **ML Anomaly Adapter**: A clean decoupling layer consuming anomaly predictions from Manan's models without binding backend logic to specific ML implementations.
...
* **Backend API & Real-Time Ingestion**: FastAPI services, SQLite/PostgreSQL persistence, and WebSocket real-time delivery.

---

## 7. Development Status

* **Current Phase**: `Phase 0 — Repository and Architecture Initialization`
* **Phase Objective**: Establish repository conventions, clean folder structure, explicit data contracts, development rules, and dependency definitions.
* **Next Phase**: `Phase 1 — FastAPI Backend Skeleton` (to be started only upon explicit instruction).
